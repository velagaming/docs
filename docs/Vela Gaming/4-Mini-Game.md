...
When the game provider has the result return, it will call Result API. The score gain by player on for that game will be returned. **Operator needs to implement this API for each game result**.
...
| game_code    | string | Game code is referred to [Game List](#game-list) API.                      |
| score        | uint64 | Score gained by the player in that game.                                   |
| total_score  | uint64 | Accumulated score gained by the player in that session.                    |
