...
| game_code    | string | Game code is referred to [Game List](#game-list) API<b>游戏代码可参考[游戏列表](#game-list-游戏列表)API                                                                                                                                                                                                                                                          |
| total_used   | uint64 | (Optional) _This is an additional parameter that will only present in action games (i.e. Blue Ocean, Safari Hunter, and Bird of Paradise)._<b>(选择性）这是一个仅在动作游戏中显示的附加参数（例如，蓝海龙王，黄金猎人和捕鸟天堂）。 <br> Total **POINTS** used (in Cents) in action games.<b>动作游戏中使用的总**点数**(进位两位数，分钱）。 <br> This field records how much credit a player has used in the action game. <b>这些记录玩家在动作游戏中使用了多少钱币。|
